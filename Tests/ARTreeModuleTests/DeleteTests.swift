...
final class ARTreeDeleteTests: CollectionTestCase {
  ...
  func testDeleteCompressPreservesParentPrefix() throws {
    ...
    t.insert(key: [1, 2, 99, 0, 0], value: 3)

    t.delete(key: [1, 2, 99, 0, 0])

    expectEqual(
      t.description,
      "○ Node4 {childs=1, partial=[]}\n" +
      "└──○ 1: Node4 {childs=2, partial=[2, 3, 10]}\n" +
      "│  ├──○ 20: 5[1, 2, 3, 10, 20] -> 1\n" +
      "│  └──○ 30: 5[1, 2, 3, 10, 30] -> 2")

    // The surviving keys must still be reachable.
    expectEqual(t.getValue(key: [1, 2, 3, 10, 20]), 1)
    expectEqual(t.getValue(key: [1, 2, 3, 10, 30]), 2)
  }
  ...
  func testDeleteAbsentKeyDoesNotRemovePresentKey() throws {
    ...
    // Sanity: the crafted key is genuinely absent.
    expectEqual(t.getValue(key: [1, 2, 5, 4, 5]), nil)

    t.delete(key: [1, 2, 5, 4, 5])

    // Deleting an absent key must leave both real keys intact.
    expectEqual(t.getValue(key: [1, 2, 3, 4, 5]), 1)
    expectEqual(t.getValue(key: [1, 2, 3, 4, 6]), 2)
    expectEqual(
      t.description,
      "○ Node4 {childs=1, partial=[]}\n" +
      "└──○ 1: Node4 {childs=2, partial=[2, 3, 4]}\n" +
      "│  ├──○ 5: 5[1, 2, 3, 4, 5] -> 1\n" +
      "│  └──○ 6: 5[1, 2, 3, 4, 6] -> 2")
  }
}
